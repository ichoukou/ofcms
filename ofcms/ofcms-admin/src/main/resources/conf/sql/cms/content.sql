#sql("query")
	select  
			 t.content_id,
		 	 ...
		 	 t.remark,
		 	 t.is_top,
		 	 CONCAT("/cms",f.value )as thumbnail,
		 	 CONCAT("/cms/industry-c-",t.content_id) as jump_url
	from
		  of_cms_content t  left join of_cms_column c on t.column_id = c.column_id
		  left join of_cms_content_field f on t.content_id = f.content_id
		  where t.status = '1'  and t.site_id = #para(site_id)
	...
	#if (sort?? && field) order by order_field order_sort  #else order by  t.is_top  desc ,t.content_id desc #end
#end
